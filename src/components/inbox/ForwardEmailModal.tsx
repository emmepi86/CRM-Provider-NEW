...
import type { ReceivedEmail, EmailAttachmentRequest } from '../../types/inbox';
...
export const ForwardEmailModal: React.FC<ForwardEmailModalProps> = ({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const result = await inboxAPI.forwardMessage(message.id, {
        to_email: to.trim(),
        body_html: body,
      ...
      console.error('Errore inoltrando email:', error);
      alert('Errore durante l\'inoltro dell\'email');
    } finally {
      setSending(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">
            Inoltra: {message.subject || '(Nessun oggetto)'}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-4">
          ...
          <div className="bg-gray-50 p-3 rounded border border-gray-200">
            ...
            <p className="text-sm text-gray-700">
              <strong>Da:</strong> {message.from_name || message.from_email}<br />
              <strong>Data:</strong> {new Date(message.received_at).toLocaleString('it-IT')}<br />
          ...
          <div>
            ...
            <button
              type="button"
              onClick={() => document.getElementById('file-upload-forward')?.click()}
          ...
          {/* Body editor (optional message) */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Messaggio aggiuntivo: <span className="text-gray-500 text-xs">(opzionale)</span>
            </label>
            <TinyMCEEditor
              value={body}
              onChange={setBody}
              height={250}
              placeholder="Aggiungi un messaggio prima dell'email inoltrata..."
              mode="email"
            />
          </div>
        </div>
